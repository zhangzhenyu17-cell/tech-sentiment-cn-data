...
def test_current_snapshot_requires_exact_official_anchor_match() -> None:
    ...
    validated = validate_live_snapshot(
        _snapshot(symbols), anchor_symbols=symbols, market_session_date="2026-09-28"
    )
    assert validated["universe_mode"].eq("point_in_time").all()
    assert validated["effective_start"].astype(str).str.startswith("2026-09-28").all()

    changed = list(symbols)
    changed[-1] = "999999"
    with pytest.raises(ValueError, match="differs from frozen"):
        validate_live_snapshot(
            _snapshot(tuple(changed)), anchor_symbols=symbols, market_session_date="2026-09-28"
        )
...
def test_public_contract_is_raw_only_and_manual_only() -> None:
    ...
    assert payload["prospective_freeze_date"] == PROSPECTIVE_FREEZE_DATE
    assert payload["first_prospective_market_session_rule"] == FIRST_PROSPECTIVE_MARKET_SESSION_RULE
    assert payload["timing"]["trading_calendar_source"] == "akshare.tool_trade_date_hist_sina"
    assert "trading_calendar_dates.csv" in payload["public_files"]
