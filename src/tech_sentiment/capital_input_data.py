...
def normalize_sse_etf_share_snapshot(
    ...
    *,
    ...
    out["fund_code"] = out["基金代码"].astype(str).str.extract(r"(\d+)", expand=False).str.zfill(6)
    ...
    if out.empty:
        return pd.DataFrame(columns=[
            "date", "fund_code", "fund_shares", "unit", "source_identity",
            "source_url", "provider_interface", "evidence_available_date",
        ])
    if out[["fund_code", "fund_shares"]].isna().any().any() or (out["fund_shares"] <= 0).any():
...
def fetch_sse_etf_share_history(
    *,
    ...
    for value in pd.DatetimeIndex(pd.to_datetime(list(trading_dates), errors="raise")).normalize().unique():
        ...
        try:
            ...
            if len(normalized):
                ...
                errors.append({"date": str(pd.Timestamp(value).date()), "error": "NO_MATCHING_ETF_ROW"})
        except Exception as exc:
            errors.append({"date": str(pd.Timestamp(value).date()), "error": f"{type(exc).__name__}: {exc}"})
        if sleep_seconds > 0:
            time.sleep(sleep_seconds)
    data = pd.concat(data_parts, ignore_index=True) if data_parts else pd.DataFrame(columns=[
        "date", "fund_code", "fund_shares", "unit", "source_identity",
        "source_url", "provider_interface", "evidence_available_date",
    ])
    return EtfShareFetchResult(data=data, errors=pd.DataFrame(errors, columns=["date", "error"]))
...
def qualify_trailing_etf_coverage(
    ...
    *,
    ...
    cal = pd.DatetimeIndex(pd.to_datetime(list(trading_dates), errors="raise")).normalize().sort_values().unique()
    x = history.copy()
    x["date"] = pd.to_datetime(x["date"], errors="raise").dt.normalize()
    x["fund_code"] = x["fund_code"].astype(str).str.extract(r"(\d+)", expand=False).str.zfill(6)
    ...
    aligned["coverage"] = aligned["observed"].astype(float).rolling(window, min_periods=window).mean()
    aligned["eligible"] = aligned["coverage"].ge(min_coverage) & aligned["fund_shares"].notna()
    aligned["fund_code"] = code
    return aligned.reset_index()[["date", "fund_code", "fund_shares", "observed", "coverage", "eligible"]]


def normalize_sse_a_share_turnover(frame: pd.DataFrame, *, observation_date: object) -> dict[str, object]:
...
def normalize_szse_a_share_turnover(frame: pd.DataFrame, *, observation_date: object) -> dict[str, object]:
    required = {"证券类别", "成交金额"}
    missing = required - set(frame.columns)
    if missing:
    ...
    categories = {"主板A股", "创业板A股", "中小板"}
    selected = x[x["证券类别"].isin(categories)]
    if selected.empty or selected["成交金额"].isna().any() or (selected["成交金额"] < 0).any():
        raise ValueError("SZSE A-share turnover rows are missing or invalid")
    names = set(selected["证券类别"])
    if "主板A股" not in names or "创业板A股" not in names:
        raise ValueError("SZSE overview lacks required main-board/ChiNext A-share rows")
    return {
        "date": _date(observation_date),
        "szse_a_share_turnover_yuan": float(selected["成交金额"].sum()),
...
def combine_sse_szse_a_share_turnover(sse: pd.DataFrame, szse: pd.DataFrame) -> pd.DataFrame:
    ...
    out = left[["date", "sse_a_share_turnover_yuan"]].merge(
        right[["date", "szse_a_share_turnover_yuan"]], on="date", how="inner", validate="one_to_one"
    )
    out["amount"] = pd.to_numeric(out["sse_a_share_turnover_yuan"], errors="coerce") + pd.to_numeric(
        out["szse_a_share_turnover_yuan"], errors="coerce"
    )
...
def qualify_financing_yuan(
    ...
    *,
    ...
    required = {
        "date", "sse_financing_balance", "szse_financing_balance",
        "sse_source_unit", "szse_source_unit",
    }
    missing = required - set(frame.columns)
    if missing:
    ...
    units_ok = x["sse_source_unit"].eq("yuan") & x["szse_source_unit"].eq("yuan")
    for col in ("sse_financing_balance", "szse_financing_balance"):
        x[col] = pd.to_numeric(x[col], errors="coerce")
    numeric = x.dropna(subset=["sse_financing_balance", "szse_financing_balance"])
    in_range = (
        numeric["sse_financing_balance"].between(min_plausible_yuan, max_plausible_yuan)
        & numeric["szse_financing_balance"].between(min_plausible_yuan, max_plausible_yuan)
    )
    if not units_ok.all():
        return pd.DataFrame(), {"state": "QUARANTINED_UNIT_UNVERIFIED", "n": int(len(x))}
    if numeric.empty or not in_range.all():
        return pd.DataFrame(), {"state": "QUARANTINED_UNIT_MISMATCH", "n": int(len(numeric))}
    med_sse = float(numeric["sse_financing_balance"].median())
    med_sz = float(numeric["szse_financing_balance"].median())
    ratio = max(med_sse / med_sz, med_sz / med_sse)
    if not np.isfinite(ratio) or ratio > max_cross_exchange_ratio:
        return pd.DataFrame(), {"state": "QUARANTINED_UNIT_MISMATCH", "n": int(len(numeric)), "median_scale_ratio": ratio}
    out = numeric[["date", "sse_financing_balance", "szse_financing_balance"]].copy()
    out["financing_balance_yuan"] = out["sse_financing_balance"] + out["szse_financing_balance"]
    out["unit"] = "yuan"
    out["sse_source_identity"] = SSE_MARGIN_SOURCE_ID
    out["szse_source_identity"] = SZSE_MARGIN_SOURCE_ID
    return out.reset_index(drop=True), {"state": "CANONICAL_UNIT_QUALIFIED", "n": int(len(out)), "median_scale_ratio": ratio}
...
def fetch_sse_szse_a_share_turnover_history(
    *,
    ...
    errors: list[dict[str, str]] = []
    dates = pd.DatetimeIndex(pd.to_datetime(list(trading_dates), errors="raise")).normalize().sort_values().unique()
    for value in dates:
        date_arg = pd.Timestamp(value).strftime("%Y%m%d")
        try:
            sse_rows.append(normalize_sse_a_share_turnover(sse_fetcher(date_arg), observation_date=value))
        except Exception as exc:
            errors.append({"date": str(pd.Timestamp(value).date()), "exchange": "SSE", "error": f"{type(exc).__name__}: {exc}"})
        try:
            szse_rows.append(normalize_szse_a_share_turnover(szse_fetcher(date_arg), observation_date=value))
        except Exception as exc:
            errors.append({"date": str(pd.Timestamp(value).date()), "exchange": "SZSE", "error": f"{type(exc).__name__}: {exc}"})
        if sleep_seconds > 0:
            time.sleep(sleep_seconds)
    sse = pd.DataFrame(sse_rows)
    szse = pd.DataFrame(szse_rows)
    combined = combine_sse_szse_a_share_turnover(sse, szse) if len(sse) and len(szse) else pd.DataFrame(
        columns=["date", "sse_a_share_turnover_yuan", "szse_a_share_turnover_yuan", "amount", "scope", "canonical_all_a_state"]
    )
