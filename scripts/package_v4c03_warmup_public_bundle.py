...
WARMUP_START = "2021-01-04"
WARMUP_END = "2021-06-14"

PRODUCER_FILES = (
    ".github/workflows/v4c03-02-phase-a-warmup.yml",
    "reference/v4c03_phase_a_warmup_public_universe_v1.json",
    "data/reference/kc50_anchor_2026-09-14.csv",
    "data/reference/v4c03_kc50_adjustments_2021warmup_2026.csv",
    ...
    "scripts/package_v4c03_warmup_public_bundle.py",
...
def _producer_identity(repo_root: Path) -> dict[str, object]:
    ...
    for relative in PRODUCER_FILES:
        ...
        if not path.is_file():
            raise FileNotFoundError(f"V4C-03 warm-up producer file missing: {relative}")
        rows.append(
            {
                "path": relative,
                "sha256": _sha(path),
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Package the immutable public-only V4C-03 Phase A warm-up bundle."
    )
    ...
    if not receipt_path.is_file():
        raise FileNotFoundError("V4C-03 Phase A warm-up receipt missing")
    receipt = json.loads(receipt_path.read_text(encoding="utf-8"))
    if receipt.get("status") != "PUBLIC_PHASE_A_WARMUP_ASSEMBLED":
        raise ValueError("V4C-03 Phase A warm-up receipt status mismatch")
    if receipt.get("phase") != "A_WARMUP_ONLY":
        raise ValueError("V4C-03 Phase A warm-up phase mismatch")
    if receipt.get("start_date") != WARMUP_START or receipt.get("end_date") != WARMUP_END:
        raise ValueError("V4C-03 Phase A warm-up receipt window mismatch")
    if receipt.get("sample_eligibility") is not False:
        raise ValueError("V4C-03 warm-up rows must not be sample eligible")
    if receipt.get("public_only") is not True:
        raise ValueError("V4C-03 Phase A warm-up receipt is not public-only")
    for key in (
        ...
        if receipt.get(key) is not False:
            raise ValueError(f"V4C-03 warm-up public boundary drift: {key}")
    ...
    base = (
        f"v4c03-phase-a-warmup-public-universe-20210104-20210614-"
